############  RFSub1 on Kaggle using Random Forest  ################
...
X <-read.csv(file="train_comp1_2020.csv", stringsAsFactors = F)
Xt <-read.csv(file="test_comp1_2020.csv", stringsAsFactors = F)
...
# head(Xt$CreditScore)
# Xt$CreditScore <- Xt$CreditScore %>% replace(.=="NULL", 1) # replace with 1
# head(Xt$CreditScore)
...
head(Xt$S063)
Xt$S063 <- Xt$S063 %>% replace(.=="NULL", 0) # replace with 0
head(Xt$S063)
...
# Build the model
agri_model <- randomForest(PastDue ~ CreditScore + TYCornUnits + TYBeanUnits + TYWheatUnits +
                                LYCornUnits + LYBeanUnits + AT36 + G093,
                                data = X, ntree=200, importance=T)

# Predict using the Xt set
prediction <- predict(agri_model, Xt)
# Save the solution to a dataframe with two columns
RFSub1<- data.frame(id = Xt$id, PastDue = prediction)
# Write the output to file
write.csv(RFSub1, file = 'RFSub1.csv', row.names = F)
